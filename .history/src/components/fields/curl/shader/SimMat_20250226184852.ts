...
export default class SimMatCurly extends ShaderMaterial {
  constructor(pos: Float32Array, offset: Float32Array) {
    const posTex = new DataTexture(pos, 512, 512, RGBAFormat, FloatType);
    posTex.needsUpdate = true;
    const offTex = new DataTexture(offset, 512, 512, RGBAFormat, FloatType);
    offTex.needsUpdate = true;
    super({
      uniforms: {
        uPositions: { value: posTex },
        uPositions2: { value: offTex },
        uTime: { value: 0 },
      },
      vertexShader: /* glsl */ `
        varying vec2 vUv;
