...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    void main(){
      ...
      vec4 pos = texture2D( uPositions, uv -0.5);
      vec4 offset = texture2D( uOffset, uv -0.5);
      vec4 ip = pos;
    ...
    // float sc2 = map(sin(time * 2. * PI) , -2.,2.,-2.,1.);
    // float amp2 = map(sin(time * 2. * PI) , -2.,2.,1.,30.);
    // float n3;
    // for(int i = 0; i<4; i++){
    //   n3 += cnoise(vec3(pos.xy * vec2(floor(sc2),1.) , uTime*0.5) + vec3(0.,uTime*0.1,0.)) * 5. * amp2;
    // }
  
    // float n = cnoise(vec3(pos.xy * vec2(0.5,1.) , uTime*0.75)) * 10.;
