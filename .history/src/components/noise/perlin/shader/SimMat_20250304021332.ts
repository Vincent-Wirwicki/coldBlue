...
float sdCircle( vec2 p, float r )
{
    ...
    float mlb = map(time, 0.,1.,0.15,0.75);
    float mlb2 = map(time, 0.,1.,0.75,0.15);
    float box = sdBox(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime*2. , .001) , vec2(time2 * 2. +0.2)) ;
    float box2 = sdBox(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime*2. , .001) , vec2(mlb2)) ;
    float triangle = sdEquilateralTriangle(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime , .01) + vec2(0.,0.25), 0.5);
    float circle = sdCircle(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime , .01), mlb);
    float mixGeo = mix(length(box), length(circle), repeat);
    float boundBox = (4. * (box*0.5));
     float boundBox2 = (4. * (box2*0.5));
    float n3;
    for(int i = 0; i<4; i++){
    ...
    vel *= (n3  *0.075 * n )  * boundBox2 * (8. * (circle*0.15));
