...
float sdEquilateralTriangle( in vec2 p, in float r )
{
    ...
    float box = sdBox(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime*2. , .001) , vec2(0.75)) ;
      float triangle = sdEquilateralTriangle(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime , .01) - vec2(0.15), 0.5);
      float mixGeo = mix(length(box), length(triangle), time2*time2);
    ...
    vel *= (n3  *0.075 * n ) *length(box)*length(triangle) ;
