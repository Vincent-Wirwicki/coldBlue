...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    float amp2 = map(sin(time * 2. * PI) , -2.,2.,10.,30.);
    float box = sdBox((pos.xy) - smoothstep(1.5, 1.25, length(pos.xy -0.5)  ) , vec2(0.25)) ;
    float boundBox = (10. * (box*0.0015));
    float n3;
    for(int i = 0; i<4; i++){
      n3 += cnoise(vec3(pos.xy * vec2(floor(sc2),1.5) - (1. + boundBox ) , uTime*0.5) + vec3(0.,uTime*0.1,0.)) * 15. * amp2;
    }

    float n = cnoise(vec3(pos.xy * vec2(0.5) - (1. + boundBox) , uTime*0.15)) * 10.;
    vel *= (n3  *0.075 * n ) * (1./boundBox) ;
