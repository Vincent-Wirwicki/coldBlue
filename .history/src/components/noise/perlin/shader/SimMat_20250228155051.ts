...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    float sc = map(sin(time2 * 2. * PI)*0.1 , -4.,4.,-2.,2.);
    float sc2 = map(sin(time * 2. * PI)*0.5, -4.,4.,-1.,1.);
    float amp2 = map(sin(time * 2. * PI) , -2.,2.,15.,30.);
    float n3;
   
    for(int i = 0; i<4; i++){
      n3 += cnoise(vec3(pos.xy * vec2(sc,1.) , uTime*0.5)) * 1. * amp2 * float(i);
    }
  
    float box = sdBox((pos.xy) - smoothstep(1.5, 1.25, length(pos.xy -0.5)  ) , vec2(0.15)) ;
    float boundBox = (10. * (box*0.015));
    float n = cnoise(vec3( pos.xy * vec2(floor(1.), uTime*0.1 ) + vec2(1., uTime), 1.)) * 10. ;
    vel *= n  *0.075 *n3  ;
    
     pos.x += vel.x * cos(n  *n3) * 0.1  * boundBox;
     pos.y += vel.y * sin(n  *n3) * 0.1  * boundBox;
