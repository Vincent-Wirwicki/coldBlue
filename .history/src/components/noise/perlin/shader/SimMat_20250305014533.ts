...
float tetrahedronSDF(vec3 p, float h)  {
    ...
    float box = sdBox(sdPos , vec2(sc2, 0.1)) ;
    float box2 = sdBox(sdPos , vec2(time*0.015 , time*0.25 + 0.5)) ;
    float triangle = sdEquilateralTriangle(sdPos, mlb2);
    float circle = sdCircle(sdPos, mlb);
    ...
    float n = cnoise(vec3(pos.xy * vec2(sc2,.75)  , uTime*0.5)) * 10.;
    vel *= (n3  *0.075 * n )  * box * triangle ;
