...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    float n3;
    for(int i = 0; i<4; i++){
      n3 += cnoise(vec3(pos.xy * vec2(floor(sc2),1.5) + boundBox  , uTime*0.5) + vec3(0.,uTime*0.1,0.)) * 15. * amp2;
    }

    float n = cnoise(vec3(pos.xy * vec2(0.5) + boundBox , uTime*0.15)) * 10.;
    vel *= n3  *0.075 * fract(n)  *boundBox ;
