...
float map(in float v, in float iMin, in float iMax, in float oMin, in float oMax) { return oMin + (oMax - oMin) * (v - iMin) / (iMax - iMin); }
    ...
    float sc2 = map(sin(time * 2. * PI) , -2.,2.,-4.,4.);
    float amp2 = map(sin(time * 2. * PI) , -2.,2.,1.,30.);
    float n3;
    for(int i = 0; i<4; i++){
      n3 += cnoise(vec3(pos.xy * vec2(floor(sc),1.) , uTime*0.5) + vec3(0.,uTime*0.1,0.)) * 5. * amp2;
    }
  
    float n = cnoise(vec3(pos.xy * vec2(floor(sc2),1.) , uTime*0.75)) * 10.;
