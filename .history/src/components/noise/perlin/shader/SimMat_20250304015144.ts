...
float sdCircle( vec2 p, float r )
{
    ...
    // float box = sdBox((pos.xy) - smoothstep(1.5, 1.25, length(pos.xy -0.5)  ) , vec2(0.25)) ;
    // float boundBos = (10. * (box*0.015));
    // float n3;
    // for(int i = 0; i<4; i++){
    //   n3 += cnoise(vec3(pos.xy * vec2(floor(sc2),1.5)   , uTime*0.5) + vec3(0.,uTime*0.1,0.)) * 15. * amp2;
    // }

    // float n = cnoise(vec3(pos.xy * vec2(0.5) , uTime*0.15)) * 10.;
    // vel *= n3  *0.075 *n   ;
    ...
    // float sc2 = map(sin(time * 2. * PI) *0.5, -2.,2.,-8.,8.);
    ...
    // float n2 = cnoise(vec3( pos.xy * vec2(floor(sc2),1.)  , uTime*0.5)) * 40. * amp2;
    // vel *= n  *0.05 *n3 ;
    ...
    float box = sdBox(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime*2. , .001) , vec2(0.75)) ;
      float triangle = sdEquilateralTriangle(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime , .01) + vec2(0.,0.25), 0.5);
      float circle = sdCircle(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime , .01) + vec2(0.,0.25), 0.5);
      float mixGeo = mix(length(box),mix(length(triangle), length(circle), time2) , time2*time2);
    float boundBox = (4. * (box*0.15));
    float n3;
    for(int i = 0; i<4; i++){
      
      n3 += cnoise(vec3(pos.xy * vec2(0.,2.)  , uTime*0.75) + vec3(0.,uTime*0.1,0.)) * 15. * amp2;
    }

    float n = cnoise(vec3(pos.xy * vec2(.15,.75)  , uTime*0.5)) * 10.;
    vel *= (n3  *0.075 * n ) *mixGeo ;
