...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    // 2 Jumpy Perlin---------------
    float box = sdBox((pos.xy) * smoothstep(0.9,0.955, radius ) , vec2(0.1)) ;
    ...
    vel *= n3  *0.075 *n   ;
    pos.x += vel.x * cos(n * n3) * 0.1 * smoothstep(0.15,0.75,length(radius - 0.5));
    pos.y += vel.y * sin(n * n3) * 0.1 * smoothstep(0.15,0.75,length(radius - 0.5));
      //---------------------------
    ...
    // float sc = map(sin(time2 * 2. * PI) , -8.,8.,-2.,2.);
    // float sc2 = map(sin(time * 2. * PI), -4.,4.,-1.,1.);
    // float amp2 = map(sin(time * 2. * PI) , -2.,2.,1.,10.);
    // float n3;
    // for(int i = 0; i<4; i++){
    //   n3 += cnoise(vec3(pos.xy * vec2(0.,1.) , uTime*0.5)) * 1. * amp2 * float(i);
    // }
  
    // float box = sdBox(pos.xy , vec2(0.5));
    // float n = cnoise(vec3( pos.xy * vec2(floor(sc), 1. ) + vec2(1., uTime), 1.)) * 10. ;
    // vel *= n  *0.05 *n3 ;
    
    //   pos.xy *= smoothstep(0.15,0.75,box + (radius - 0.5));
    //  pos.x += vel.x * cos(n  ) * 0.075 ;
    //  pos.y += vel.y * sin(n  ) * 0.075 ;
