...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    float sc2 = map(sin(time * 2. * PI)*0.25 , -2.,2.,-2.,2.);
    float amp2 = map(sin(time * 2. * PI) , -2.,2.,10.,30.);
    float box = sdBox(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime , .001) , vec2(0.5)) ;
    float boundBox = (10. * (box*0.15));
    float n3;
    float n4 = cnoise(vec3(pos.xy * vec2(0.,1.5)  , uTime*0.5) + vec3(0.,uTime*0.1,0.)) * 15. * amp2
    // for(int i = 0; i<4; i++){
      
    //   n3 += cnoise(vec3(pos.xy * vec2(0.,1.5)  , uTime*0.5) + vec3(0.,uTime*0.1,0.)) * 15. * amp2;
    // }

    float n = cnoise(vec3(pos.xy * vec2(0.5)  , uTime*0.15)) * 10.;
    vel *= (n4  *0.075 * n ) *boundBox ;4

    pos.x += vel.x * cos(n * n4) * 0.1 ;
    pos.y += vel.y * sin(n * n4) * 0.1 ;
