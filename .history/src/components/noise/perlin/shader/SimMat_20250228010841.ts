  ...
  constructor(pos: DataTexture, offset: DataTexture) {
    ...
    // pos.y += vel.y * sin(n  * n2) * 0.75;
      //---------------------------
          ...
          float sc = map(sin(time2 * 2. * PI)*0.1 , -1.,1.,.5,1.);
    float sc2 = map(sin(time * 2. * PI) *0.5, -2.,2.,-1.,1.);
    float amp2 = map(sin(time * 2. * PI) , -2.,2.,1.,10.);
    float n3;
    for(int i = 0; i<4; i++){
      n3 += cnoise(vec3(pos.xy * vec2(1.,floor(sc)) , 1.)) * 1. * amp2 * float(i);
    }
  
    float n = cnoise(vec3( pos.xy *1. + vec2(1., uTime *0.75), 1.)) * 10. ;
    float n2 = cnoise(vec3( pos.xy * vec2(1.,floor(sc2))  , 1.)) * 40. * amp2;
    vel *= n  *0.05 *n3 ;
    pos.x += vel.x * cos(n  * n3) * 0.075;
    pos.y += vel.y * sin(n  * n3) * 0.75;
