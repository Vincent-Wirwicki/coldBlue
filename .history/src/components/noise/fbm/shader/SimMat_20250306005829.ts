...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    float n3 = fbm(vec3((pos.xy) * vec2(2., .5)- vec2(1., uTime*0.1), uTime*0.5)   ) *3.  ;

  
    float n = noise(vec3(pos.xy * vec2(2.,  4.) + vec2(1., uTime*0.25) , uTime*0.5)   ) * amp2 * 2.;
    // float n2 = snoise(vec3(pos.xy + vec2(1., uTime*0.5) , uTime*0.75)   )*.5;
    vel *= n  *0.5 *n3 * length(box);
