...
float sdCircle( vec2 p, float r )
{
    ...
    float box = sdBox(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime*2. , offset.z *0.01) , vec2(time + 0.25 , .5)) ;
    float circle = sdCircle(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime*2. , offset.z *0.01),  time2 + 0.5);
    float circle2 = sdCircle(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime*2. , offset.z *0.01),  time2 + 0.15);
    ...
    vel *= n  *0.5 *n3 * (length(circle))*(length(circle));
