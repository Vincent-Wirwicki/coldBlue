...
float sdCircle( vec2 p, float r )
{
    ...
    float circle = sdCircle(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime*2. , offset.z *0.01),  mix(0.1,0.3,time));
    float circle2 = sdCircle(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime*2. , offset.z *0.01),  mix(0.5,0.7,time));
    float boundBox = (4. * (box*0.5));
    float n3, c2;
    for(int i = 0; i<4; i++){
      n3 += snoise(vec3((pos.xy) * vec2(time2, .5)- vec2(1., uTime*0.1), uTime*0.5)   ) * amp2 ;
      c2 = sdCircle(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime*2. , offset.z *0.01),  mix(0.6,0.9,time*time*10.));
    }
