...
float sdEquilateralTriangle( in vec2 p, in float r )
{
    ...
    void main(){
      ...
      float time2 = mod(mod(uTime*0.75, 1.0) + 1.0, 1.0);
    ...
    // float sc2 = map(sin(time * 2. * PI) , -2.,2.,.5,-2.);
    // float amp2 = map(sin(time * 2. * PI) , -2.,2.,1.,10.);

    // float n3;
    // for(int i = 0; i<8; i++){
    //   n3 += snoise(vec3((pos.xy) * vec2(floor(sc2), 1.)- vec2(1., uTime*0.1), uTime*0.5)   ) * 1. * amp2 ;
    // }
  
    // float n = snoise(vec3(pos.xy * vec2(1.,  floor(sc)) + vec2(1., uTime*0.25) , uTime*0.15)   ) * amp2 * 2.;
    ...
    float n3;
    for(int i = 0; i<4; i++){
      n3 += snoise(vec3((pos.xy) * vec2(floor(sc2), .5)- vec2(1., uTime*0.1), uTime*0.5)   ) * amp2 ;
    }
