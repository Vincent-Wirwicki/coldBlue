...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    float box = sdBox(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime*2. , offset.z *0.01) , vec2(0.25)) ;
    float boundBox = (4. * (box*0.5));
    float n3;
    for(int i = 0; i<4; i++){
      n3 += snoise(vec3((pos.xy) * vec2(floor(sc2), 1.)- vec2(1., uTime*0.1), uTime*0.5)   ) * amp2 ;
    }
