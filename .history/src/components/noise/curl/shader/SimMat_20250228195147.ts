...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    float boundBox = (4. * (box*0.5));
      vec2 vel = offset.xy;
      vec3 nc = curl(vec3(pos.x*1.25,pos.y*.85 * smoothstep(0.5,0.,length(pos.xy)), uTime *0.1) + vec3(0.,0.,0.))*4.   ;
      // nc += curl(vec3(pos.xy*2.,mod(uTime, offset.z)) )*.5;
      // nc += curl(vec3(pos.xy*4.,mod(uTime, offset.z)) )*.25;

      // nc += curl(vec3(pos.xy*0.25,1.) )*2.5;
      float angle = atan(pos.x, pos.y);
      float an = atan(nc.x, nc.y);
      float d2 = length(nc.xy - pos.xy) *0.015;
      vel *= nc.xy *0.5 * length(box) ;
