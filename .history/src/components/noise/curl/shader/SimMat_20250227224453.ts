...
float noiseX(vec2 p){
    ...
    void main(){
      ...
      vec2 vel = offset.xy;
      vec3 nc = curl(vec3(pos.x*2.25,pos.y*2.5, uTime *0.1) + vec3(0.,uTime*0.1,0.))*2.5   ;
      // nc += curl(vec3(pos.xy*0.5,1.) )*5.;
      // nc += curl(vec3(pos.xy*0.25,1.) )*2.5;
      float angle = atan(pos.x, pos.y);
      float an = atan(nc.x, nc.y);
      float d2 = length(nc.xy - pos.xy) *0.015;
      vel *= nc.xy *0.75 ;
      pos.x += vel.x + normalize(nc.x) * 0.2* smoothstep(0.,0.15,d2)   ;
      pos.y += vel.y + normalize(nc.y) * 0.2* smoothstep(0.,0.15,d2) ;
