...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    void main(){
      ...
      float box = sdBox(pos.xy - smoothstep(1., 0., length(pos.xy -0.5) ) - mod(uTime , .01) , vec2(0.5)) ;
      vec2 vel = offset.xy;
      ...
      pos.x += vel.x * normalize(nc.x) * smoothstep(0.,0.15,pow(.2,d2))   ;
      pos.y += vel.y * normalize(nc.y) * smoothstep(0.,0.15,pow(.2,d2)) ;
