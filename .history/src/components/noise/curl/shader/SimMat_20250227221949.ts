...
float noiseX(vec2 p){
    ...
    void main(){
      ...
      vec2 vel = offset.xy;
      vec3 nc = curl(vec3(pos.x*.75,pos.y*.5, uTime*0.075))*5.   ;
      vec3 nc1 = curl(vec3(nc.xy*2.,1.) +vec3(10.,5.,1.))*1.;
      vec3 nc2 = curl(vec3(nc1.xy*4.,1.) +vec3(10.,5.,1.))*.5;

      float an = atan(nc2.x, nc2.y);
      vel *= nc2.xy *0.2 +nc1.xy;
