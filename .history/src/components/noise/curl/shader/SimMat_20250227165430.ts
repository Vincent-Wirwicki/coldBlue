...
float noiseX(vec2 p){
    ...
    float n3;
    for(int i = 0; i<4; i++){
      // n3 += noiseX(vec3(pos.xy * vec2(floor(sc2),1.) , uTime*0.5) + vec3(0.,uTime*0.1,0.)) * 5. * amp2;
      n3 += noiseX(vec2(pos.xy * vec2(0.,floor(sc)) + vec2(1., uTime*0.1))) * 5. * amp2;
    }
  
    // float n = noiseX(vec3(pos.xy * vec2(0.,1.) , uTime*0.75)) * 1.;
    float n = noiseX(vec2(pos.xy * vec2(0.,1.) + vec2(1., uTime *0.2))) * 1.;


    vel *= n3  *0.075 *n;
    pos.x += vel.x * cos(50. * n3) * 0.085;
    pos.y += vel.y * sin(n * n3) * 0.085;
