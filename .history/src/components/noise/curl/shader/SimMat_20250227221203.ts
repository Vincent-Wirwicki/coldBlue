...
export default class SimMatCurly extends ShaderMaterial {
  constructor(pos: DataTexture, offset: DataTexture) {
    ...
    void main(){
      ...
      vec2 vel = offset.xy;
      vec3 nc = curl(vec3(pos.x*1.75,pos.y*.5, uTime*0.1) + vec3(1., uTime*0.1,1.))*10.   ;
      float an = atan(nc.x, nc.y);
      vec2 nnn = normalize(nc.xy - pos.xy);
      vel *= nnn.xy *0.2 ;
      pos.x += vel.x * cos(angle * an - uTime *0.1) * 0.5   ;
      pos.y += vel.y * sin(angle * an - uTime *0.1) * 0.5  ;
