...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    float boundBox = (4. * (box*0.5));
      vec2 vel = offset.xy;
      vec3 nc = curl(vec3(pos.x,pos.y, uTime *0.1) + vec3(0.,0.,0.))*1.   ;
      nc += curl(vec3(pos.xy*2.,1.) + vec3(0.,mod(uTime*0.5, offset.z),0.))*.5;
      nc += curl(vec3(pos.xy*4.,1.) + vec3(0.,uTime*0.25,0.) )*.25;

      // nc += curl(vec3(pos.xy*0.25,1.) )*2.5;
      float angle = atan(pos.x, pos.y);
      float an = atan(nc.x, nc.y);
      float d2 = length(nc.xy - pos.xy) *0.015;
      vel *= nc.xy *0.75 * length(box) ;
      pos.x += vel.x * normalize(nc.x) * smoothstep(0.,0.1,d2  )   ;
      pos.y += vel.y * normalize(nc.y) * smoothstep(0.,0.1,d2 ) ;
