...
float noiseX(vec2 p){
    ...
    void main(){
      ...
      vec2 vel = offset.xy;
      vec3 nc = curl(vec3(pos.x*.25,pos.y*.5, uTime*0.075))*2.5   ;
      vec3 nc1 = curl(vec3(pos.xy*0.5,1.) )*5.;
      vec3 nc2 = curl(vec3(pos.xy*0.25,1.) )*2.5;
      float angle = atan(pos.x, pos.y);
      float an = atan(nc2.x, nc2.y);
      vel *= nc.xy *0.2 ;
      pos.x += vel.x + normalize(pos.x - nc.x)*0.015   ;
      pos.y += vel.y + normalize(pos.y - nc.y)*0.015  ;
