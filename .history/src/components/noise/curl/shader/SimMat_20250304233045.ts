...
float sdBox( in vec2 p, in vec2 b )
{
    ...
    void main(){
      ...
      float box = sdBox(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime , .01) + vec2(0.1), vec2(0.5,0.05)) ;
            float box2 = sdBox(pos.xy - smoothstep(1.9999, 1.9998, length(pos.xy -0.5) ) - mod(uTime , .01) - vec2(0.1), vec2(0.5,0.05)) ;

      vec2 vel = offset.xy;
      vec3 nc = curl(vec3(pos.x*1.25,pos.y*.85, uTime *0.25 + length(box)) + vec3(0.,uTime*0.25,0.))*8.   ;
      float d2 = length(nc.xy - pos.xy) *0.015;
      vel *= nc.xy *0.15 * length(box) * length(box2 * 0.5);
      pos.x += vel.x * normalize(nc.x) * smoothstep(0.,0.15,d2)   ;
      pos.y += vel.y * normalize(nc.y) * smoothstep(0.,0.15,d2) ;
