  ...
  return (
    <main className="absolute top-0 left-0 w-screen h-screen flex justify-center items-center py-8 bg-black">
      <div className="relative grid grid-cols-6 grid-rows-6 gap-2 w-4/5 h-full bg-black ">
        ...
        <div className="col-start-5 row-start-6 text-neutral-500">Github</div>
        <div className="col-start-1 self-end justify-self-end rotate-[-90deg]">
          Project
        </div>
...
const Nav = ({
  ...
  return (
    <nav className="col-start-2 col-span-4 grid grid-cols-subgrid  row-start-1 self-end text-end text-neutral-200 w-full">
      {/* <h1 className="">Lab</h1> */}
...
const MyLink = ({
  ...
  return (
    <NavLink
      className={`[&.active]:bg-[#00FFFF] [&.active]:no-underline [&.active]:text-black text-end line-through tracking-widest
 col-start-${col + 1}`}
      to={path}
    >
