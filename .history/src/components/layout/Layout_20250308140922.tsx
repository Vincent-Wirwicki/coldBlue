  ...
  return (
    <main className="absolute top-0 left-0 w-screen h-screen flex justify-center items-center  py-4">
      <div className="relative grid grid-cols-6 grid-rows-6 gap-2 w-4/5 h-full">
        ...
        <h1 className="col-start-3 row-start-6 grid grid-cols-subgrid self-start text-end text-neutral-500">
          <span>cold</span> <span className="inline col-start-4">blue</span>
        </h1>
        <Nav paths={paths} />
...
const MyLink = ({
  ...
  return (
    <NavLink
      className={`[&.active]:text-[#00FFFF] [&.active]:no-underline text-end line-through flex justify-between items-baseline
 col-start-${col + 1}`}
      to={path}
    >
      <span className="w-[10px] h-[10px]  [&.active]:bg-[#00FFFF] bg-neutral-500"></span>
      <span>
        {col + 1} | {title}
      </span>
    </NavLink>
