  ...
  return (
    <main className="absolute top-0 left-0 w-screen h-screen flex justify-center items-center  py-4">
      <div className="relative grid grid-cols-6 grid-rows-6 gap-1 w-4/5 h-full">
        ...
        <h1 className="col-start-2 self-end text-end">cold blue</h1>
...
const Nav = ({
  ...
  return (
    <nav className="col-start-4 self-end text-neutral-500 flex flex-col  items-end gap-5  ">
...
const MyLink = ({ path, title }: { path: string; title: string }) => {
  return (
    <NavLink
      className="[&.active]:text-[#00FFFF] [&.active]:underline  line-through "
      to={path}
    >
      {title}
    </NavLink>
